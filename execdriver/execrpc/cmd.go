...
func NewCmdDriver(stdin bool) (*CmdDriver, error) {
	realDriver, err := native.NewDriver("/var/lib/docker")
	if err != nil {
		return nil, err
	}
...
func (d *CmdDriver) Start(wrapper *CommandWrapper, res *int) error {
	...
	go func() {
		stdin := utils.NewReadCloserWrapper(cmd.Stdin, func() error { return nil })
		pipes := execdriver.NewPipes(stdin, cmd.Stdout, cmd.Stderr, d.stdin)
